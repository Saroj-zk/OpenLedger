...
/* How far the list travels while the user reads it, in px. */
const SCROLL_TO = 1040;
...
function applySwitch(s, t, T, target) {
  ...
  if (t >= T.menuOpen && t < T.pick) {
    ...
    else if (t < T.scrollDown[1]) s.menuScroll = ease(span(T.scrollDown, t)) * SCROLL_TO;
    else if (t < T.scrollUp[0]) s.menuScroll = SCROLL_TO;
    else if (t < T.scrollUp[1]) s.menuScroll = SCROLL_TO * (1 - ease(span(T.scrollUp, t)));
    else s.menuScroll = 0;
  }
...
const TOKEN = {
  ...
  T: {
    ...
    menuOpen: 5900,
    scrollDown: [6300, 8100],
    scrollUp: [8300, 9000],
    cursorToClaude: [9100, 9600],
    pick: 9600,
    type2: [10200, 11600],
    send2: 11700,
    think2: [11700, 12150],
...
const MEMORY = {
  ...
  T: {
    ...
    menuOpen: 5600,
    scrollDown: [5900, 6600],
    scrollUp: [6700, 7100],
    cursorToClaude: [7200, 7650],
    pick: 7650,
    type2: [8100, 9500],
    send2: 9600,
    recall: [9700, 10400],
...
const SCRIPTS = {
  token: { ...TOKEN, derive: makeDerive(TOKEN) },
  memory: { ...MEMORY, derive: makeDerive(MEMORY) },
};
                        ...
                        >
                          ...
                          <span className="relative block h-[250px] overflow-hidden">
                            ...
                            <span className="pointer-events-none absolute bottom-1 right-1 top-1 w-[3px] rounded-full bg-[color:var(--color-border)]">
                              <span
                                className="absolute left-0 h-[24%] w-full rounded-full bg-[color:var(--rule-strong)]"
                                style={{ top: `${(s.menuScroll / SCROLL_TO) * 76}%` }}
                              />
                            </span>
                          </span>
                        </span>
